...
class PerWellAnalyzerEnhanced:
    ...
    def _calculate_light_response(self, well_data):
        ...
        results = []
        for metric in baseline['Metric'].unique():
            base_vals = baseline[baseline['Metric'] == metric]['Value']
            stim_vals = stim[stim['Metric'] == metric]['Value']
            ...
            light_code = stim['LIGHT_CODE'].iloc[0] if len(stim) > 0 else 'UNKNOWN'
            drug = stim['DRUG'].iloc[0] if len(stim) > 0 else 'NONE'
...
class SpontaneousAnalyzer:
    ...
    def analyze(self):
        """Spontaneous activity 분석 - BASELINE만"""
        print('\n[1] Spontaneous Activity Analysis (BASELINE ONLY)...')
        ...
        for well in sorted(baseline['Well'].unique()):
            ...
            for metric in sorted(well_data['Metric'].unique()):
                metric_data = well_data[well_data['Metric'] == metric]
                
                results.append({
                    'Well': well,
                    'Plate_ID': metric_data['Plate_ID'].iloc[0],
                    'Metric': metric,
                    'Mean': metric_data['Value'].mean(),
                    'Std': metric_data['Value'].std(),
                    'Count': len(metric_data),
                    'DIFF_DAY': metric_data['DIFF_DAY'].mean(),
                    'Drug': metric_data['DRUG'].iloc[0]
                })
...
class LightResponseAnalyzer:
    ...
    def _analyze_per_well(self):
        ...
        print('  Adding per-well analysis...')
        ...
        for well in sorted(self.result_df['Well'].unique()):
            ...
            for metric in ['mean_firing_rate_hz', 'burst_frequency_hz', 'number_of_spikes']:
                metric_data = well_data[well_data['Metric'].str.contains(metric, case=False, na=False)]
                if not metric_data.empty:
                    per_well_results.append({
                        ...
                        'Light_Code': metric_data['Light_Code'].iloc[0]
                    })
        ...
        if not self.per_well_df.empty:
            ...
            print(f'  ✓ Per-well analysis saved: {csv_path.name}')
...
class DrugEffectAnalyzer:
    ...
    def _analyze_baseline_effect(self):
        """Baseline drug effect"""
        print('  Analyzing baseline drug effect...')
        ...
        for well in sorted(set(control['Well'].unique()) & set(drug['Well'].unique())):
            ...
            for metric in sorted(set(well_ctrl['Metric'].unique()) & set(well_drug['Metric'].unique())):
                ctrl_vals = well_ctrl[well_ctrl['Metric'] == metric]['Value']
                drug_vals = well_drug[well_drug['Metric'] == metric]['Value']
                ...
                drug_name = well_drug['DRUG'].iloc[0]
                ...
                results.append({
                    ...
                    'Difference': difference,
                    'Pct_Change': pct_change,
                    'Plate_ID': well_ctrl['Plate_ID'].iloc[0]
                })
        ...
        if not self.baseline_df.empty:
            ...
            print(f'  ✓ Baseline drug effects saved: {csv_path.name}')
    ...
    def _analyze_light_modulation(self):
        ...
        for well in wells:
            ...
            for metric in sorted(set(cb['Metric'].unique()) & set(cs['Metric'].unique()) & 
                                set(db['Metric'].unique()) & set(ds['Metric'].unique())):
                # Control response
                ctrl_resp = cs[cs['Metric'] == metric]['Value'].mean() - cb[cb['Metric'] == metric]['Value'].mean()
                
                # Drug response
                drug_resp = ds[ds['Metric'] == metric]['Value'].mean() - db[db['Metric'] == metric]['Value'].mean()
                
                # Modulation
                modulation = drug_resp - ctrl_resp
                pct_change = (modulation / ctrl_resp * 100) if ctrl_resp != 0 else 0
                
                light_code = cs['LIGHT_CODE'].iloc[0] if len(cs) > 0 else 'UNKNOWN'
                drug_name = db['DRUG'].iloc[0] if len(db) > 0 else 'UNKNOWN'
        ...
        if not self.light_modulation_df.empty:
            ...
            print(f'  ✓ Light response modulation saved: {csv_path.name}')
