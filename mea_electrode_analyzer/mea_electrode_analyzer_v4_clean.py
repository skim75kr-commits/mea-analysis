...
def load_single_electrode_excel(path: str) -> pd.DataFrame:
    ...
    # Well_Info: DIV가 이미 계산되어 있으면 직접 사용, 없으면 계산
    if "DIV" in df_well.columns:
        div_map = dict(zip(df_well["Well"], df_well["DIV"]))
    else:
        div_map = None
    diff_map = dict(zip(df_well["Well"], df_well["Differentiation_Day"]))
    # days_post_plating: 플레이팅 후 실험까지 경과일
    days_post_plating = meta.get("DAYS_POST_PLATING",
                                  meta.get("days_post_plating",
    ...
    for _, row in df_template.iterrows():
        ...
        for col in electrode_cols:
            ...
            # DIV: Well_Info에 이미 계산된 값이 있으면 사용, 없으면 계산
            diff_day0 = diff_map.get(well, np.nan)
            if div_map is not None:
                div = div_map.get(well, np.nan)
            else:
                div = (diff_day0 + days_post_plating
